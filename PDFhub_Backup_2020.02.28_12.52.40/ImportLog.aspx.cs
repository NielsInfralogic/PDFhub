...
namespace PDFhub
{
    public partial class ImportLog : System.Web.UI.Page
    {
        ...
        private void RebindLogGrid(bool callRebind)
        {
            ...
            if (db.GetLastLogItems(ref logItems, Utils.ReadConfigInt32("MaxLogItemsToShow", 500), false, Models.LogItemSource.Import, out string errmsg) == false)
            {
        ...
        protected void RadGridLog_ItemCommand(object sender, GridCommandEventArgs e)
        {
            ...
            if (e.CommandName == "Retry")
            {
                ...
                {
                    DataProviders.DBaccess db = new DataProviders.DBaccess();
                    db.RetryFile(Models.ServiceType.PlanImport, fileName, inputQueue, out string errmsg);
